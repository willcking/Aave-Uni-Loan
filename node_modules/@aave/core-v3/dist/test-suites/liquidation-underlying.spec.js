...
(0, make_suite_1.makeSuite)('Pool Liquidation: Liquidator receiving the underlying asset', (testEnv) => {
    ...
    it("It's not possible to liquidate on a non-active collateral or a non active principal", async () => {
        const { configurator, weth, pool, users: [, user], dai, } = testEnv;
        await configurator.setReserveActive(weth.address, false);
        await (0, chai_1.expect)(pool.liquidationCall(weth.address, dai.address, user.address, ethers_1.utils.parseEther('1000'), false)).to.be.revertedWith('27');
    ...
    it('Liquidates the borrow', async () => {
        const { dai, weth, users: [, borrower, , liquidator], pool, oracle, helpersContract, faucetMintable, } = testEnv;
        ...
        (0, chai_1.expect)(ethReserveDataAfter.totalLiquidity).to.be.closeTo(ethReserveDataBefore.totalLiquidity.sub(expectedCollateralLiquidated), 2, 'Invalid collateral total liquidity');
        ...
        (0, chai_1.expect)(ethReserveDataAfter.availableLiquidity).to.be.closeTo(ethReserveDataBefore.availableLiquidity.sub(expectedCollateralLiquidated), 2, 'Invalid collateral available liquidity');
    });
    it('User 3 deposits 1000 USDC, user 4 0.06775 WETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
        const { usdc, users: [, , , depositor, borrower, liquidator], pool, oracle, weth, helpersContract, faucetMintable, } = testEnv;
        ...
        const principalDecimals = (await helpersContract.getReserveConfigurationData(usdc.address))
            .decimals;
        const expectedCollateralLiquidated = principalPrice
            .mul(ethers_1.BigNumber.from(amountToLiquidate))
            .percentMul(10500)
            .mul(ethers_1.BigNumber.from(10).pow(collateralDecimals))
            .div(collateralPrice.mul(ethers_1.BigNumber.from(10).pow(principalDecimals)));
    ...
    it('User 4 deposits 0.033 AAVE - drops HF, liquidates the AAVE, which results on a lower amount being liquidated', async () => {
        ...
        await faucetMintable.mint(aave.address, borrower.address, await (0, contracts_helpers_1.convertToCurrencyDecimals)(aave.address, '0.033'));
        //approve protocol to access the borrower wallet
        await aave.connect(borrower.signer).approve(pool.address, constants_1.MAX_UINT_AMOUNT);
        //borrower deposits 1 AAVE
        const amountToDeposit = await (0, contracts_helpers_1.convertToCurrencyDecimals)(aave.address, '0.033');
        await pool
            .connect(borrower.signer)
        ...
        const principalDecimals = (await helpersContract.getReserveConfigurationData(usdc.address))
            .decimals;
        const expectedCollateralLiquidated = constants_1.oneEther.mul('33').div('1000');
        const expectedPrincipal = collateralPrice
            .mul(expectedCollateralLiquidated)
            .mul(ethers_1.BigNumber.from(10).pow(principalDecimals))
            .div(principalPrice.mul(ethers_1.BigNumber.from(10).pow(collateralDecimals)))
        ...
        (0, chai_1.expect)(aaveReserveDataAfter.totalLiquidity).to.be.closeTo(aaveReserveDataBefore.totalLiquidity.sub(expectedCollateralLiquidated), 2, 'Invalid collateral total liquidity');
        ...
        (0, chai_1.expect)(aaveReserveDataAfter.availableLiquidity).to.be.closeTo(aaveReserveDataBefore.availableLiquidity.sub(expectedCollateralLiquidated), 2, 'Invalid collateral available liquidity');
    });
});
