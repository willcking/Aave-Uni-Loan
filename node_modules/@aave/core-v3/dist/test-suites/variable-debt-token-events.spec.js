...
const printBalance = async (name, debtToken, userAddress) => {
    console.log(name, 'balanceOf', await hardhat_1.ethers.utils.formatEther(await debtToken.balanceOf(userAddress)), 'scaledBalance', await hardhat_1.ethers.utils.formatEther(await debtToken.scaledBalanceOf(userAddress)));
};
...
const updateBalances = (balances, variableDebtToken, receipt) => {
    var _a, _b, _c;
    let events = (0, tokenization_events_1.getVariableDebtTokenEvent)(variableDebtToken, receipt, 'Mint');
    for (const ev of events) {
        balances.balance[ev.onBehalfOf] = (_a = balances.balance[ev.onBehalfOf]) === null || _a === void 0 ? void 0 : _a.add(ev.value);
    }
    events = (0, tokenization_events_1.getVariableDebtTokenEvent)(variableDebtToken, receipt, 'Burn');
    for (const ev of events) {
        balances.balance[ev.from] = (_b = balances.balance[ev.from]) === null || _b === void 0 ? void 0 : _b.sub(ev.value.add(ev.balanceIncrease));
        balances.balance[ev.from] = (_c = balances.balance[ev.from]) === null || _c === void 0 ? void 0 : _c.add(ev.balanceIncrease);
    }
};
(0, make_suite_1.makeSuite)('VariableDebtToken: Events', (testEnv) => {
    let alice, bob, depositor, depositor2;
    let snapId;
    before(async () => {
        const { users, pool, dai, weth, configurator, aDai, deployer, faucetMintable } = testEnv;
    ...
    const testMultipleBorrowsAndRepays = async (indexChange) => {
        const { pool, dai, variableDebtDai, weth, faucetMintable } = testEnv;
        let rcpt;
        ...
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, alice, dai.address, '100', alice.address, DEBUG);
        ...
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, alice, dai.address, '50', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.repayVariableBorrow)(pool, alice, dai.address, '20', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.repayVariableBorrow)(pool, alice, dai.address, '10', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.repayVariableBorrow)(pool, alice, dai.address, '220', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (DEBUG) {
            await printBalance('alice', variableDebtDai, alice.address);
        }
        // Check final balances
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, alice, dai.address, '1', alice.address);
        updateBalances(balances, variableDebtDai, rcpt);
    ...
    const testMultipleBorrowsAndRepaysOnBehalf = async (indexChange) => {
        ...
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, bob, dai.address, '100', bob.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.repayVariableBorrow)(pool, alice, dai.address, '150', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.repayVariableBorrow)(pool, alice, dai.address, '50', bob.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, alice, dai.address, '10', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        updateBalances(balances, variableDebtDai, rcpt);
        const aliceBalanceAfter = await variableDebtDai.balanceOf(alice.address);
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, bob, dai.address, '1', bob.address);
        updateBalances(balances, variableDebtDai, rcpt);
    ...
    const testMultipleBorrowsOnBehalfAndRepaysOnBehalf = async (indexChange) => {
        ...
        await variableDebtDai.connect(alice.signer).approveDelegation(bob.address, constants_1.MAX_UINT_AMOUNT);
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, bob, dai.address, '100', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.variableBorrow)(pool, bob, dai.address, '50', bob.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
        ...
        rcpt = await (0, tokenization_events_1.repayVariableBorrow)(pool, alice, dai.address, '250', alice.address, DEBUG);
        updateBalances(balances, variableDebtDai, rcpt);
        if (indexChange) {
            log('- Increase index due to great borrow of DAI');
            await increaseSupplyIndex(pool, faucetMintable, depositor, weth.address, dai.address);
